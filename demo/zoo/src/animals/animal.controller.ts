...
export class AnimalController {
    ...
    @Delete(":id")
    async deleteAnimal(@Param() id: number): Promise<Result> {
        await this.animalService.deleteAnimal(id);
        return { code: 200, message: "删除动物成功" };
    }

    @Put(":id")
    async updateAnimal(@Param() id: number, @Body() animal: Animal): Promise<Result> {
        await this.animalService.updateAnimal(id, animal);
        return { code: 200, message: "更新动物成功" };
    }

    @Get(":id")
    async findOneAnimal(@Param() id: number): Promise<Result> {
